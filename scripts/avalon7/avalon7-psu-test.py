...
# Support Canaan's AvalonMiner761 Power test
#
...
def set_vol_value(q, vol):
    ...
    crc8 = crc8_mult_byte(i2c_value)
    print("crc8 value: 0x%x" %(crc8))
...
def test_polling(q):
    while (True):
        val = raw_input("Please input: s for set voltage, h for help:")
        if (val == 's') or (val == 'S'):
            vol = raw_input("Please input setting voltage value:")
            if (vol == '1'):
                psu_enable(q)
            elif (vol == '2'):
                psu_open(q)
